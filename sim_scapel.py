...
# Bumped from 10x10x4 to 20x20x6 for denser mesh -> cleaner, more precise
# surgical cuts. Wall spacing along X/Y drops from ~11mm to ~5.3mm, so
# _walk_and_sever's per-cell wall severing resolves cut paths at roughly
# 2x finer granularity than before.
SOFT_RES_X   = 20   # particles along X -> spacing = 2*0.05/19 ~= 5.3 mm
SOFT_RES_Y   = 20   # particles along Y
...
class SoftBodyCube:
    ...
    def __init__(
        ...
        # Same aliasing fix as _vel_list above: build one independent list
        # per triangle rather than repeating a single shared reference.
        self._tri_color_list = [list(SKIN_TISSUE_COLOR) for _ in range(len(oriented))]
...
class WarpSoftBodySim:
    ...
    def load_example_assets(self):
        ...
        probe.CreateDoubleSidedAttr(True)
        ...
        # Flat/matte material -- no specular highlight, no clearcoat, no
        # "shine". A mesh with no material explicitly bound otherwise
        # picks up the renderer's default preview material, which DOES
        # include a specular response under the dome light, giving the
        # tissue a glossy/plastic look in renders and screenshots. Fully
        # rough + non-metallic + zero specular removes that highlight, so
        # figures show flat tissue color only. The per-face skin/muscle
        # displayColor primvar still drives the diffuse albedo (via a
        # primvar-reader node), so cut visualization is unaffected.
        mat_path = Sdf.Path(SOFT_BODY_PRIM_PATH + "/MatteMaterial")
        ...
        color_reader.CreateInput("fallback", Sdf.ValueTypeNames.Color3f).Set(
            Gf.Vec3f(*SKIN_TISSUE_COLOR.tolist()))
        ...
        UsdShade.MaterialBindingAPI.Apply(cb.GetPrim()).Bind(material)
