...
class SoftBodyCube:
    ...
    def _rebuild_boundary_tris(self, tets_np):
        ...
        loop -- at ~11k tets (4 faces each = ~43k raw faces before
        ...
        is_bnd = np.asarray(self._is_boundary_list, dtype=bool)
        all_original = (is_bnd[oriented[:, 0]] & is_bnd[oriented[:, 1]]
                         & is_bnd[oriented[:, 2]])
        skin_np, muscle_np = (SKIN_TISSUE_COLOR.astype(np.float32),
                               MUSCLE_TISSUE_COLOR.astype(np.float32))
        colors = np.where(all_original[:, None], skin_np[None, :], muscle_np[None, :])
